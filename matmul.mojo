...
def run_matmul_python_jit() -> Float64:
    let pymatmul: PythonObject = Python.import_module("pymatmul")
    let py = Python.import_module("builtins")

    let gflops = pymatmul.benchmark_matmul_python_jit(M, N, K).to_float64()
    py.print(py.str("{:<13}{:>8.3f} GFLOPS").format("Python JIT:", gflops))

    return gflops

def run_matmul_jnp() -> Float64:
    let pymatmul: PythonObject = Python.import_module("pymatmul")
    let py = Python.import_module("builtins")
    
    let gflops = pymatmul.benchmark_matmul_jnp(M, N, K).to_float64()
    py.print(py.str("{:<13}{:>8.3f} GFLOPS").format("JNP:", gflops))

    return gflops

def run_matmul_jnp_jit() -> Float64:
    let pymatmul: PythonObject = Python.import_module("pymatmul")
    let py = Python.import_module("builtins")
    
    let gflops = pymatmul.benchmark_matmul_jnp_jit(M, N, K).to_float64()
    py.print(py.str("{:<13}{:>8.3f} GFLOPS").format("JNP JIT:", gflops))
...
fn main() raises:
    ...
    #let jit_gflops = run_matmul_python_jit()
    let jnp_gflops = run_matmul_jnp()
    let jnpjit_gflops = run_matmul_jnp_jit()
    ...
    bench[matmul_naive, "Naive:"](jnp_gflops, jnpjit_gflops)
    bench[matmul_vectorized, "Vectorized: "](jnp_gflops, jnpjit_gflops)
    bench[matmul_parallelized, "Parallelized:"](jnp_gflops, jnpjit_gflops)
    bench[matmul_tiled, "Tiled:"](jnp_gflops, jnpjit_gflops)
    bench[matmul_unrolled, "Unrolled:"](jnp_gflops, jnpjit_gflops)
    bench[matmul_accumulated, "Accumulated:"](jnp_gflops, jnpjit_gflops)
